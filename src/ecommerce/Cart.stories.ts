/**
 * Cart Stories - WenderMedia Astro Components
...
- Free shipping progress bar
  ...
  argTypes: {
    position: { control: 'select', options: ['left', 'right'], description: 'Drawer position' },
    showTax: { control: 'boolean', description: 'Show tax calculation' },
    isEmpty: { control: 'boolean', description: 'Show empty state' },
  },
};
...
const renderCart = (args: Record<string, unknown>) => html`
  <style>
  ...
  <div class="cart-demo">
    <div class="cart-header">
      <h2 class="cart-title">Warenkorb <span class="cart-count">${args.isEmpty ? '(0)' : '(2)'}</span></h2>
    ...
    ` : html`
      ...
      <div class="cart-footer">
        ${args.showTax ? html`<div class="cart-row"><span>inkl. MwSt. (19%)</span><span>4,75 &euro;</span></div>` : ''}
...
export const WithItems: Story = {
  args: { position: 'right', showTax: true, isEmpty: false },
  render: (args) => renderCart(args),
};

export const EmptyState: Story = {
  args: { position: 'right', showTax: true, isEmpty: true },
  render: (args) => renderCart(args),
};
