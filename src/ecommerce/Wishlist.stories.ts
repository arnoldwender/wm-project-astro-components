...
const renderWishlistToggle = (args: Record<string, unknown>) => html`
  ...
  <div style="padding: 2rem; font-family: system-ui, sans-serif; display: flex; align-items: center; gap: 1rem;">
    <button class="wl-toggle" aria-label="${args.isActive ? 'Aus Wunschliste entfernen' : 'Zur Wunschliste hinzufügen'}" aria-pressed="${args.isActive}">
    ...
    <span style="font-size: 0.875rem; color: #6b7280;">${args.isActive ? 'In der Wunschliste' : 'Zur Wunschliste hinzufügen'}</span>
  </div>
`;
