/**
 * LocationCard Stories - WenderMedia Astro Components
...
- Featured badge and location tags
  ...
  argTypes: {
    name: { control: 'text', description: 'Location name' },
    address: { control: 'text', description: 'Street address' },
    variant: { control: 'select', options: ['default', 'card', 'compact', 'horizontal', 'detailed'], description: 'Card variant' },
    featured: { control: 'boolean', description: 'Featured badge' },
  },
};
...
const renderLocationCard = (args: Record<string, unknown>) => html`
  <div style="padding: 2rem; max-width: 24rem;">
    <div style="background: #fff; border: ${args.featured ? '2px solid #3b82f6' : '1px solid #e2e8f0'}; border-radius: 0.75rem; overflow: hidden; ${args.featured ? 'position: relative;' : ''}">
      ${args.featured ? html`<div style="position: absolute; top: 1rem; right: 1rem; padding: 0.125rem 0.5rem; background: #3b82f6; color: #fff; font-size: 0.625rem; font-weight: 600; border-radius: 9999px; z-index: 1;">HQ</div>` : ''}
      <div style="aspect-ratio: 16/9; background: linear-gradient(135deg, #e2e8f0, #cbd5e1);"></div>
      <div style="padding: 1.5rem;">
        <h3 style="font-weight: 600; color: #1e293b; margin: 0 0 0.5rem;">${args.name}</h3>
        <p style="font-size: 0.875rem; color: #64748b; margin: 0 0 1rem;">${args.address}</p>
        ${args.phone ? html`
          <div style="display: flex; align-items: center; gap: 0.5rem; font-size: 0.875rem; color: #64748b; margin-bottom: 0.5rem;">
            ...
            <a href="tel:${args.phone}" style="color: #64748b; text-decoration: none;">${args.phone}</a>
          </div>
        ` : ''}
        ${args.email ? html`
          <div style="display: flex; align-items: center; gap: 0.5rem; font-size: 0.875rem; color: #64748b; margin-bottom: 1rem;">
            ...
            <a href="mailto:${args.email}" style="color: #64748b; text-decoration: none;">${args.email}</a>
          </div>
        ` : ''}
        ${args.tags ? html`
...
export const Default: Story = {
  args: { name: 'Berlin Headquarters', address: 'Friedrichstrasse 123, 10117 Berlin', variant: 'card', featured: true, phone: '+49 30 12345678', email: 'berlin@acmecorp.com', tags: ['HQ', 'Sales'] },
  render: (args) => renderLocationCard(args),
};

export const Simple: Story = {
  args: { name: 'Halle Office', address: 'Marktplatz 1, 06108 Halle (Saale)', variant: 'card', featured: false, phone: '+49 345 9876543' },
  render: (args) => renderLocationCard(args),
};
