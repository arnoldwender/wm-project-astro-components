/**
 * MaintenanceSection Stories - WenderMedia Astro Components
...
const meta: Meta = {
  title: 'Sections/MaintenanceSection',
  tags: ['autodocs'],
  parameters: {
    docs: {
      description: {
...
- Terminal-style developer variant with animated cursor
  ...
  argTypes: {
    variant: { control: 'select', options: ['default', 'progress', 'scheduled', 'minimal', 'branded', 'technical'], description: 'Visual variant' },
    title: { control: 'text', description: 'Main heading' },
    description: { control: 'text', description: 'Description text' },
    estimatedTime: { control: 'text', description: 'Estimated downtime' },
    progress: { control: { type: 'range', min: 0, max: 100 }, description: 'Progress percentage' },
    contactEmail: { control: 'text', description: 'Support email' },
  },
};
...
const renderMaintenance = (args: Record<string, unknown>) => html`
  <section style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:2rem;background:#f8fafc;font-family:system-ui,sans-serif;">
    <div style="width:100%;max-width:40rem;text-align:center;">
      ...
      <h1 style="font-size:2rem;font-weight:700;margin:0 0 1rem;color:#1e293b;">${args.title}</h1>
      <p style="font-size:1.125rem;color:#64748b;margin:0 0 2rem;line-height:1.6;">${args.description}</p>
      ${args.estimatedTime ? html`
        <div style="display:inline-flex;align-items:center;gap:0.5rem;padding:0.75rem 1.25rem;background:#f1f5f9;border-radius:9999px;font-size:0.875rem;color:#64748b;margin-bottom:1.5rem;">
          ...
          <span>Estimated downtime: <strong>${args.estimatedTime}</strong></span>
        </div>
      ` : ''}
      ${args.contactEmail ? html`
        <div style="display:flex;justify-content:center;gap:1.5rem;margin-top:1rem;">
          <a href="mailto:${args.contactEmail}" style="display:inline-flex;align-items:center;gap:0.5rem;color:#3b82f6;text-decoration:none;font-size:0.875rem;font-weight:500;">Contact Support</a>
        </div>
      ` : ''}
    </div>
  </section>
...
const renderProgress = (args: Record<string, unknown>) => html`
  <section style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:2rem;background:#f8fafc;font-family:system-ui,sans-serif;">
    <div style="width:100%;max-width:40rem;text-align:center;">
      <div style="position:relative;width:160px;height:160px;margin:0 auto 2rem;">
        <svg viewBox="0 0 100 100" style="transform:rotate(-90deg);">
          <circle cx="50" cy="50" r="45" fill="none" stroke="#e2e8f0" stroke-width="8"/>
          <circle cx="50" cy="50" r="45" fill="none" stroke="#3b82f6" stroke-width="8" stroke-linecap="round" stroke-dasharray="283" stroke-dashoffset="${283 - (283 * (args.progress as number)) / 100}"/>
        </svg>
        <span style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);font-size:2rem;font-weight:700;color:#1e293b;">${args.progress}%</span>
      </div>
      ...
      <div style="max-width:20rem;margin:0 auto 1.5rem;">
        <div style="height:0.5rem;background:#e2e8f0;border-radius:9999px;overflow:hidden;margin-bottom:0.5rem;">
          <div style="height:100%;background:#3b82f6;border-radius:9999px;width:${args.progress}%;"></div>
        </div>
        <span style="font-size:0.75rem;color:#94a3b8;">${args.progress}% Complete</span>
      </div>
    </div>
  </section>
`;
