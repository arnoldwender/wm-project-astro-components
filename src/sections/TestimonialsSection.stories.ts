/**
 * TestimonialsSection Stories - WenderMedia Astro Components
...
const meta: Meta = {
  ...
  tags: ['autodocs'],
  parameters: { docs: { description: { component: 'Social-proof section with six layout variants (grid, carousel, featured, masonry, quote, wall). Configurable columns, carousel navigation, masonry CSS columns layout.' } } },
  argTypes: { variant: { control: 'select', options: ['grid', 'carousel', 'featured', 'masonry', 'quote', 'wall'] }, columns: { control: { type: 'range', min: 2, max: 4 } } },
};
export default meta;
type Story = StoryObj;
  ...
  render: () => html`
    <section style="padding:4rem 2rem;background:#f8fafc;font-family:system-ui,sans-serif;">
      ...
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1.5rem;max-width:64rem;margin:0 auto;">
        ${[
          { name: 'Jane Doe', role: 'CEO, Acme Corp', quote: 'This tool saved us hours every week.' },
          { name: 'John Smith', role: 'CTO, Globex', quote: 'The best investment we made this year.' },
          { name: 'Maria Garcia', role: 'Designer, Initech', quote: 'Intuitive and beautiful. Highly recommended.' },
        ].map(t => html`
