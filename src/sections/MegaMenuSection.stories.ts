/**
 * MegaMenuSection Stories - WenderMedia Astro Components
...
const meta: Meta = {
  title: 'Sections/MegaMenuSection',
  tags: ['autodocs'],
  parameters: {
    docs: {
      description: {
...
- Mobile bottom-sheet fallback
  ...
  argTypes: {
    variant: { control: 'select', options: ['default', 'featured', 'tabs', 'columns', 'cards', 'full'], description: 'Layout variant' },
    trigger: { control: 'text', description: 'Trigger button text' },
  },
};
...
const renderMegaMenu = (args: Record<string, unknown>) => html`
...
export const Default: Story = {
  args: { trigger: 'Products', variant: 'default' },
  render: (args) => renderMegaMenu(args),
};

export const WithFeatured: Story = {
  args: { trigger: 'Resources', variant: 'featured' },
  render: () => html`
    <div style="position:relative;padding:2rem;font-family:system-ui,sans-serif;">
