/**
 * ContactSection Stories - WenderMedia Astro Components
...
- Map embed support
  ...
  argTypes: {
    variant: { control: 'select', options: ['split', 'centered', 'card', 'minimal', 'map'], description: 'Layout variant' },
...
const renderContact = (args: Record<string, unknown>) => html`
  <section style="padding: 4rem 2rem; background: ${args.variant === 'card' ? '#f8fafc' : '#fff'};">
    <div style="max-width: 72rem; margin: 0 auto;">
      ...
      <div style="display: grid; grid-template-columns: ${args.variant === 'split' ? '1fr 1fr' : '1fr'}; gap: 3rem; ${args.variant !== 'split' ? 'max-width: 32rem; margin: 0 auto;' : ''}">
...
export const Split: Story = {
  args: { variant: 'split', title: 'Get in Touch' },
  render: (args) => renderContact(args),
};

export const Centered: Story = {
  args: { variant: 'centered', title: 'Contact Us' },
  render: (args) => renderContact(args),
};
