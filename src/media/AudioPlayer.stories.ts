/**
 * AudioPlayer Stories - WenderMedia Astro Components
...
- Accessible and fully keyboard navigable
  ...
  argTypes: {
    ...
    showPlaylist: { control: 'boolean', description: 'Show playlist panel' },
  },
};
...
const renderAudioPlayer = (args: Record<string, unknown>) => html`
  <style>
  ...
  <div class="ap">
    <div class="ap-main">
      ...
      <div class="ap-content">
        <div>
          <h3 class="ap-title">${args.title}</h3>
          <p class="ap-artist">${args.artist}</p>
        </div>
...
export const SingleTrack: Story = {
  args: { title: 'Episode 1: Introduction', artist: 'Acme Corp Podcast', showPlaylist: false },
  render: (args) => renderAudioPlayer(args),
};

export const WithPlaylist: Story = {
  args: { title: 'Welcome', artist: 'Acme Corp', showPlaylist: true },
  render: (args) => renderAudioPlayer(args),
};
