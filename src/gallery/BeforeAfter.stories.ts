/**
 * BeforeAfter Stories - WenderMedia Astro Components
...
- Click-to-snap and drag-to-compare interactions
  ...
  argTypes: {
    beforeLabel: { control: 'text', description: 'Before label text' },
    afterLabel: { control: 'text', description: 'After label text' },
    initialPosition: { control: { type: 'range', min: 0, max: 100, step: 5 }, description: 'Initial slider position' },
    aspectRatio: { control: 'text', description: 'Aspect ratio' },
  },
};
...
const renderBeforeAfter = (args: Record<string, unknown>) => html`
  <style>
    .ba { position: relative; width: 100%; max-width: 800px; margin: 2rem auto; font-family: system-ui, sans-serif; }
    .ba-wrapper { position: relative; width: 100%; overflow: hidden; border-radius: 0.5rem; cursor: ew-resize; box-shadow: 0 10px 40px rgba(0,0,0,0.15); aspect-ratio: ${args.aspectRatio}; }
    ...
    .ba-before { background: linear-gradient(135deg, #fecaca 0%, #f87171 100%); display: flex; align-items: center; justify-content: center; color: #991b1b; font-size: 1.5rem; font-weight: 700; clip-path: inset(0 ${100 - (args.initialPosition as number)}% 0 0); }
    .ba-slider { position: absolute; top: 0; bottom: 0; left: ${args.initialPosition}%; transform: translateX(-50%); z-index: 10; pointer-events: none; }
  ...
  <div class="ba">
    <div class="ba-wrapper">
      ...
      <div class="ba-slider" role="slider" aria-label="Image comparison slider" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${args.initialPosition}" tabindex="0">
      ...
      <div class="ba-label ba-label-before">${args.beforeLabel}</div>
      <div class="ba-label ba-label-after">${args.afterLabel}</div>
    </div>
  </div>
`;

export const Default: Story = {
  args: { beforeLabel: 'Vorher', afterLabel: 'Nachher', initialPosition: 50, aspectRatio: '4/3' },
  render: (args) => renderBeforeAfter(args),
};

export const MostlyBefore: Story = {
  args: { beforeLabel: 'Before', afterLabel: 'After', initialPosition: 75, aspectRatio: '16/9' },
  render: (args) => renderBeforeAfter(args),
};

export const MostlyAfter: Story = {
  args: { beforeLabel: 'Original', afterLabel: 'Edited', initialPosition: 25, aspectRatio: '4/3' },
  render: (args) => renderBeforeAfter(args),
};
