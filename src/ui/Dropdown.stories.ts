/**
 * Dropdown Stories - WenderMedia Astro Components
...
- Reduced motion support
  ...
  argTypes: {
    ...
    placeholder: { control: 'text', description: 'Placeholder text when no selection' },
    size: {
      control: { type: 'select' },
      options: ['sm', 'md', 'lg'],
...
const renderDropdown = (args: Record<string, unknown>) => html`
  ${dropdownStyles}
  <div class="dd-root">
    ${args.label ? html`
      <label class="dd-label">${args.label}<span class="required">*</span></label>
    ` : ''}
    <button class="dd-trigger ${args.error ? 'error' : ''} ${args.disabled ? 'disabled' : ''}">
      <span class="placeholder">${args.placeholder || 'Select an option'}</span>
      <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
      </svg>
    </button>
    ${args.error ? html`<p class="dd-error" role="alert">${args.error}</p>` : ''}
    <div class="dd-panel">
      ${args.searchable ? html`
        <div class="dd-search">
...
export const Default: Story = {
  args: { label: 'Country', placeholder: 'Choose a country', size: 'md', variant: 'default', disabled: false, error: '', searchable: false, multiple: false },
  render: (args) => renderDropdown(args),
};

export const WithSearch: Story = {
  args: { label: 'Country', placeholder: 'Choose a country', size: 'md', variant: 'default', disabled: false, error: '', searchable: true, multiple: false },
  render: (args) => renderDropdown(args),
};

export const WithError: Story = {
  args: { label: 'Country', placeholder: 'Choose a country', size: 'md', variant: 'default', disabled: false, error: 'Please select a country', searchable: false, multiple: false },
  render: (args) => renderDropdown(args),
};

export const Disabled: Story = {
  args: { label: 'Country', placeholder: 'Choose a country', size: 'md', variant: 'default', disabled: true, error: '', searchable: false, multiple: false },
  render: (args) => renderDropdown(args),
};
