/**
 * Toast Stories - WenderMedia Astro Components
...
- Reduced motion support
  ...
  argTypes: {
    ...
    message: { control: 'text', description: 'Optional description below title' },
    showProgress: { control: 'boolean', description: 'Show progress bar countdown' },
    position: {
      control: { type: 'select' },
...
const toastColors: Record<string, { border: string; icon: string; progress: string }> = {
  success: { border: '#22c55e', icon: '#22c55e', progress: '#22c55e' },
  error: { border: '#ef4444', icon: '#ef4444', progress: '#ef4444' },
  warning: { border: '#f59e0b', icon: '#f59e0b', progress: '#f59e0b' },
  info: { border: '#3b82f6', icon: '#3b82f6', progress: '#3b82f6' },
};
...
const renderToast = (args: Record<string, unknown>) => {
  const type = (args.type as string) || 'info';
  const colors = toastColors[type];
  const iconPath = toastIcons[type];
  ...
  return html`
    <style>
      ...
      .toast-icon { flex-shrink: 0; width: 1.25rem; height: 1.25rem; color: ${colors.icon}; }
      ...
      .toast-progress-bar { height: 100%; width: 65%; background: ${colors.progress}; transition: width 0.3s; }
    </style>
    <div class="toast-demo" role="alert">
      <div class="toast-body">
        ...
        <div class="toast-content">
          ...
          ${args.message ? html`<div class="toast-message">${args.message}</div>` : ''}
        </div>
...
export const Success: Story = {
  args: { type: 'success', title: 'Changes saved successfully!', message: '', showProgress: true, position: 'top-right' },
  render: (args) => renderToast(args),
};

export const Error: Story = {
  args: { type: 'error', title: 'Something went wrong', message: 'Please try again later or contact support.', showProgress: true, position: 'top-right' },
  render: (args) => renderToast(args),
};

export const Warning: Story = {
  args: { type: 'warning', title: 'Your session will expire soon', message: 'Save your work to avoid losing changes.', showProgress: true, position: 'top-right' },
  render: (args) => renderToast(args),
};
