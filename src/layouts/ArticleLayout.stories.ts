/**
 * ArticleLayout Stories - WenderMedia Astro Components
...
- Related articles and comments slots
  ...
  argTypes: {
    ...
    subtitle: { control: 'text', description: 'Article subtitle' },
    readingTime: { control: 'text', description: 'Estimated reading time' },
    category: { control: 'text', description: 'Article category' },
    layout: { control: 'select', options: ['default', 'wide', 'full-width-header'], description: 'Layout variant' },
    showProgress: { control: 'boolean', description: 'Show reading progress bar' },
  },
};
...
const renderArticleLayout = (args: Record<string, unknown>) => html`
  <article style="max-width: 48rem; margin: 0 auto; padding: 2rem 1rem; font-family: system-ui, sans-serif;">
    <!-- Progress Bar -->
    ${args.showProgress ? html`<div style="position: fixed; top: 0; left: 0; height: 3px; width: 35%; background: #3b82f6; z-index: 50;"></div>` : ''}

    <!-- Header -->
    <header style="margin-bottom: 2rem;">
      ${args.category ? html`<span style="display: inline-block; padding: 0.25rem 0.75rem; font-size: 0.875rem; font-weight: 500; background: rgba(59,130,246,0.1); color: #3b82f6; border-radius: 9999px; margin-bottom: 1rem;">${args.category}</span>` : ''}
      <h1 style="font-size: 2.5rem; font-weight: 700; line-height: 1.2; margin: 0 0 1rem;">${args.title}</h1>
      ${args.subtitle ? html`<p style="font-size: 1.25rem; color: #6b7280;">${args.subtitle}</p>` : ''}
    </header>
    ...
    <div style="display: flex; align-items: center; gap: 1rem; font-size: 0.875rem; color: #9ca3af; padding-bottom: 1rem; border-bottom: 1px solid #e5e7eb; margin-bottom: 2rem;">
      ...
      ${args.readingTime ? html`<span>&middot;</span><span>${args.readingTime}</span>` : ''}
    </div>
